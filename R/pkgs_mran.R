...
#' download_pkgs_mran(date='2014-08-04', pkgs=c("plyr","ggplot2"), outdir="~/mran_snaps/")
#'
#' download_pkgs_mran(date='2014-06-19', pkgs=c("plyr_1.8.1","ggplot2_1.0.0"), outdir="~/mran_snaps/stuff/")
#' download_pkgs_mran(date='2014-06-19', pkgs="rgbif_0.6.2", outdir="~/mran_snaps")
#' }
                      ...
                      outdir=NULL, verbose=FALSE, quiet=FALSE)
{
  ...
  get_pkg_versions <- function(x){
    vers <- tryCatch(mran_pkg_versions(snapshot=snapshot_use, package=x[[1]]), 
                     error=function(e) e)
