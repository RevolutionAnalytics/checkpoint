...
setAccessDate <- function(snapshotDate, checkpointLocation="~/")
{
    ...
    cpdir <- checkpointPath(snapshotDate, checkpointLocation=checkpointLocation, type="snapshot")
...
{
    cp <- checkpointPath(NULL, checkpointLocation=checkpointLocation, type="root")
    z <- dir(cp, pattern=".{4}-.{2}-.{2}", include.dirs=TRUE, full.names=TRUE)
    sapply(z, function(x)
    {
