...
setLibPaths <- function(checkpointLocation, libPath)
{
    ...
    newLoc <- c(libPath, checkpointBasePkgs(checkpointLocation), .Library)
...
unCheckpoint <- function(new)
{
    if(!missing(new))
        warning(
            "'new' argument is no longer used; unCheckpoint will automatically revert to pre-checkpoint library paths")
...
correctR <- function(x)
{
    compareVersion(as.character(utils::packageVersion("base")), x) == 0
}
