knitr.is.installed <- function()
{
    length(find.package(package="knitr", quiet=TRUE)) > 0
}
                          ...
                          )
{
    ...
    pattern <- if(!use.knitr)
        "\\.[rR]$|\\.[rR]nw$"
    else "\\.[rR]$|\\.[rR]nw$|\\.[rR]md$|\\.[rR]pres$"
    ...
    if(length(files_k) > 0)
    {
        if(use.knitr)
        {
            ...
            else R_files <- c(files_r, files_k)

        }
        else mssg(verbose, "rmarkdown files found and will not be parsed. Set use.knitr=TRUE")
    }
    ...
    {
        z <- if(interactive())
            lapplyProgressBar(R_files, deps_by_ext, dir=dir, verbose=verbose,
                              scan.rnw.with.knitr=scan.rnw.with.knitr)
        else lapply(R_files, deps_by_ext, dir=dir, verbose=verbose,
                    scan.rnw.with.knitr=scan.rnw.with.knitr)
...
deps.R <- deps.txt <- function(file, verbose=TRUE)
{
    ...
    else list(pkgs=unique(pkgs), error=character())
}
